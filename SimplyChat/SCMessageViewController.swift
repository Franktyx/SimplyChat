...
class SCMessageViewController: JSQMessagesViewController {
    ...
    func setupFirebase() {
        ...
        messagesRef = Firebase(url: "https://simplychatapp.firebaseIO.com/messages/chatroom1")
        ...
        messagesRef.queryLimitedToNumberOfChildren(25).observeEventType(FEventType.ChildAdded, withBlock: { (snapshot) in
            ...
            if sender != nil {
                let message = SCMessage(senderId: sender!, senderDisplayName: sender!, isMediaMessage: false, hash: 0, text: text!)
...
//                    let avatarImage = JSQMessagesAvatarImageFactory.avatarWithImage(image, diameter: diameter)
